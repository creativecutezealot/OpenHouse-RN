...
export const DOWNLOAD_PROPERTIES_ATTENDE_START          = 'DOWNLOAD_PROPERTIES_ATTENDE_START';
export const DOWNLOAD_PROPERTIES_ATTENDE_SUCCESS        = 'DOWNLOAD_PROPERTIES_ATTENDE_SUCCESS';
export const DOWNLOAD_PROPERTIES_ATTENDE_FAILD          = 'DOWNLOAD_PROPERTIES_ATTENDE_FAILD';

export const DOWNLOAD_PROPERTIES_BROKER_ATTENDE_START          = 'DOWNLOAD_PROPERTIES_BROKER_ATTENDE_START';
export const DOWNLOAD_PROPERTIES_BROKER_ATTENDE_SUCCESS        = 'DOWNLOAD_PROPERTIES_BROKER_ATTENDE_SUCCESS';
export const DOWNLOAD_PROPERTIES_BROKER_ATTENDE_FAILD          = 'DOWNLOAD_PROPERTIES_BROKER_ATTENDE_FAILD';
...
export const DOWNLOAD_MLSLINK_ACCOUNT_SUCCESS        = 'DOWNLOAD_MLSLINK_ACCOUNT_SUCCESS';
...
export function resetpassword(email) {
    return (dispatch)=>{
        ...
        (async() =>{
            ...
            if(res.IsSuccess && res.data.length && res.data[0].email_sent){
...
export function authdownloadmortgage() {
    return (dispatch,getState)=>{
        ...
        let advertisingid = (getState().login.account !== null && getState().login.account.advertising_id) ? getState().login.account.advertising_id : '';
        let accountnum = (getState().login.account !== null && getState().login.account.account_num) ? getState().login.account.account_num : '';
        (async() =>{
            let res = await AuthService.authdownloadmortgage(advertisingid,accountnum);
            if(res.IsSuccess){
                dispatch({
